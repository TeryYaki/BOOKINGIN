...
<head>
    ...
    <!-- Custom Styles -->
    <style>
        /* Mencegah kedipan (flicker) Alpine.js sebelum load selesai */
...
<body class="bg-[#111] text-gray-200 font-sans min-h-screen">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex items-center gap-4">
                    <div class="flex items-center gap-2 text-sm text-gray-400">
                        <img src="https://ui-avatars.com/api/?name=Admin&background=3b82f6&color=fff" class="h-8 w-8 rounded-full border border-gray-600" alt="Admin Avatar">
    ...
    <!-- Main Content Wrapper with Alpine.js Tab Logic -->
    <!-- Gabungkan semua state (Tab & Modal) di parent wrapper ini -->
    <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8" x-data="{ activeTab: 'overview', showEditModal: false, editData: {} }">
        ...
        <div class="flex flex-wrap gap-2 mb-8 border-b border-gray-800 pb-4">
            <button @click="activeTab = 'overview'" 
                    :class="activeTab === 'overview' ? 'bg-blue-600 text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'bg-[#1b1b1b] text-gray-400 border-gray-800 hover:bg-[#222]'"
                    class="px-5 py-2.5 rounded-lg border font-medium transition duration-300 flex items-center gap-2">
                <i class="fa-solid fa-chart-pie"></i> Ringkasan
            </button>
            
            <button @click="activeTab = 'transactions'" 
                    :class="activeTab === 'transactions' ? 'bg-blue-600 text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'bg-[#1b1b1b] text-gray-400 border-gray-800 hover:bg-[#222]'"
                    class="px-5 py-2.5 rounded-lg border font-medium transition duration-300 flex items-center gap-2">
                <i class="fa-solid fa-receipt"></i> Data Transaksi
            </button>
            
            <button @click="activeTab = 'movies'" 
                    :class="activeTab === 'movies' ? 'bg-blue-600 text-white border-blue-500 shadow-lg shadow-blue-500/20' : 'bg-[#1b1b1b] text-gray-400 border-gray-800 hover:bg-[#222]'"
                    class="px-5 py-2.5 rounded-lg border font-medium transition duration-300 flex items-center gap-2">
                <i class="fa-solid fa-film"></i> Manajemen Film
            </button>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-[#1b1b1b] p-6 rounded-xl shadow-lg border-l-4 border-green-500 relative overflow-hidden group">
                    ...
                    <div class="absolute top-4 right-4 z-20">
                        <form action="{{ route('admin.dashboard') }}" method="GET" class="flex items-center gap-2 bg-black/40 backdrop-blur-md p-1.5 rounded-lg border border-white/10 shadow-sm transition-all hover:border-white/30">
                            ...
                            <div class="relative group">
                                <select name="filter_type" onchange="this.form.submit()" class="bg-transparent text-[11px] font-medium text-white pl-2 pr-5 py-1 rounded cursor-pointer focus:outline-none hover:text-green-400 transition">
                            ...
                            @if(request('filter_type') != 'yearly')
                                <div class="relative group">
                                    <select name="month" onchange="this.form.submit()" class="bg-transparent text-[11px] font-medium text-white pl-2 pr-5 py-1 rounded cursor-pointer focus:outline-none hover:text-green-400 transition">
                            ...
                            <!-- Filter Year -->
                            <div class="relative group">
                                <select name="year" onchange="this.form.submit()" class="bg-transparent text-[11px] font-medium text-white pl-2 pr-5 py-1 rounded cursor-pointer focus:outline-none hover:text-green-400 transition">
        ...
        <!-- ========================================== -->
        <div x-show="activeTab === 'transactions'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak>
        ...
        <!-- ========================================== -->
        <!-- TAB 3: MANAJEMEN FILM                      -->
        <!-- ========================================== -->
        <div x-show="activeTab === 'movies'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" x-cloak>
            
            <!-- Movie Management Section (Form + Table) -->
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                
                <!-- Tambah Film Form (Sisi Kiri) -->
        ...
        <!-- ========================================== -->
        <!-- MODAL EDIT FILM (Berlaku Global di Page)   -->
        <!-- ========================================== -->
        <div x-show="showEditModal" style="display: none;" class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" x-cloak>
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                
                <!-- Modal Overlay -->
                <div x-show="showEditModal" x-transition.opacity class="fixed inset-0 bg-black bg-opacity-80 transition-opacity" @click="showEditModal = false"></div>

                <!-- Modal Content -->
                <div x-show="showEditModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="inline-block align-bottom bg-[#1b1b1b] border border-gray-700 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form :action="'/admin/movie/' + editData.id" method="POST" enctype="multipart/form-data" class="p-6">
                        ...
                        <h3 class="text-lg font-medium leading-6 text-white mb-4">
                            <i class="fa-solid fa-edit text-blue-500 mr-2"></i> Edit Film
                        </h3>
